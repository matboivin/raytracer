...
t_mat4x4		create_camtoworld(t_vec3 from, t_vec3 to, t_vec3 world_up)
{
	...
	result.c1 = create_vec4(right.x, right.y, right.z, 0);
	result.c2 = create_vec4(up.x, up.y, up.z, 0);
	result.c3 = create_vec4(look_at.x, look_at.y, look_at.z, 0);
	result.c4 = create_vec4(from.x, from.y, from.z, 1);
	return (result);
}
